...
The export flow is also intentionally direct. The rendered favicon preview is captured in the browser with `modern-screenshot`, resized into the PNG set the app ships, packaged with `JSZip`, and downloaded locally with `file-saver`. The `.ico` file is assembled in the app as part of that same client-side pipeline. `Bun` is used for package management and scripts.
