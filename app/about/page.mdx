...
The export flow is also intentionally direct. The current editor state is rendered into canvases in the browser, packaged into the PNG set the app ships with `JSZip`, and downloaded locally with `file-saver`. The `.ico` file is assembled in the app as part of that same client-side pipeline. `Bun` is used for package management and scripts.
